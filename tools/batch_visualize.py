#!/usr/bin/env python3
"""test/audio 内の全WAVを全VADプラグインで処理してPNGを一括出力するスクリプト."""
import argparse
...
VAD_PLUGINS = ['livekit_wakeword', 'openwakeword', 'silero_vad']


def main():
    """メインエントリポイント."""
    parser = argparse.ArgumentParser(description='全WAV×全VADを一括処理してPNG出力する')
    ...
    from tools.visualize_vad import (
        build_asr_plugin, build_vad_plugin, load_wav, plot, run_pipeline,
    )
    ...
    total = len(wav_files) * len(VAD_PLUGINS)
    done = 0
    ...
    for wav_path in wav_files:
        ...
        for vad_name in VAD_PLUGINS:
            done += 1
            print(f'[{done}/{total}] {wav_stem} × {vad_name}')

            try:
                vad_plugin = build_vad_plugin(
                    vad_name, args.model_folder, None, args.threshold
                )
                asr_plugin.audio_queue = queue.Queue()
                asr_plugin.result_queue = queue.Queue()
                asr_plugin.stop_event = threading.Event()

                collected = run_pipeline(wav_path, vad_plugin, asr_plugin)
                ...
                out_path = os.path.join(out_dir, f'{wav_stem}__{vad_name}.png')
                plot(audio, collected, wav_path, vad_name, args.whisper_model, out_path)
