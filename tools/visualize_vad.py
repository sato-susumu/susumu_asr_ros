...
def build_vad_plugin(plugin_name: str, model_folder: str,
                     model_name: str | None, threshold: float):
    """VAD プラグインを構築して返す."""
    if plugin_name not in _VAD_CLASSES:
        raise ValueError(
            f"プラグイン '{plugin_name}' が見つかりません。"
            f'利用可能: {list(_VAD_CLASSES.keys())}'
        )
    import importlib
    module_path, class_name = _VAD_CLASSES[plugin_name].split(':')
...
def run_pipeline(wav_path: str, vad_plugin, asr_plugin) -> CollectedEvents:
    ...
    from susumu_asr_ros.plugin_base import (
        ASREventType, FinalResultEvent, PartialResultEvent,
        SpeechStartEvent, SpeechStopEvent, TimeoutEvent,
    )
    ...
    def on_event(event):
        et = event.event_type
        if et == ASREventType.SPEECH_START:
            assert isinstance(event, SpeechStartEvent)
            collected.vad_events.append(('vad_speech_start', event.start))
        elif et == ASREventType.SPEECH_STOP:
            assert isinstance(event, SpeechStopEvent)
            collected.vad_events.append(('vad_speech_stop', event.end))
        elif et == ASREventType.TIMEOUT:
            assert isinstance(event, TimeoutEvent)
            collected.vad_events.append(('asr_timeout', event.end))
        elif et == ASREventType.FINAL_RESULT:
            assert isinstance(event, FinalResultEvent)
            collected.final_results.append((event.start, event.end, event.text))
        elif et == ASREventType.PARTIAL_RESULT:
            assert isinstance(event, PartialResultEvent)
...
def plot(audio: np.ndarray, collected: CollectedEvents,
         wav_path: str, vad_name: str, whisper_model: str, out_path: str):
    """波形とイベントを描画して保存する."""
    ...
    title = f'{os.path.basename(wav_path)}  [VAD: {vad_name} / ASR: whisper-{whisper_model}]'
    ax.set_title(title)
    ...
    # speech_start / speech_stop ペアをスパンで塗る
    ...
    for ev, t in collected.vad_events:
        ...
        short = ev.replace('vad_', '').replace('asr_', '')
...
def main():
    ...
    parser.add_argument('--vad', default='livekit_wakeword',
                        help='VADプラグイン名 (default: livekit_wakeword)')
    parser.add_argument('--model-folder', default='models',
                        help='VADモデルフォルダ (default: models)')
    parser.add_argument('--model-name', default=None,
                        help='VADモデルファイル名 (省略時はプラグインのデフォルト)')
    ...
    print(f'VAD プラグイン初期化中: {args.vad}')
    vad_plugin = build_vad_plugin(args.vad, args.model_folder, args.model_name, args.threshold)

    print(f'Whisper 初期化中: {args.whisper_model}')
    asr_plugin = build_asr_plugin(args.whisper_model, args.language, args.device)

    print('パイプライン実行中...')
    collected = run_pipeline(args.wav, vad_plugin, asr_plugin)

    print(f'  VAD イベント: {len(collected.vad_events)} 件')
    for ev, t in collected.vad_events:
        print(f'    {t:.3f}s  {ev}')
    ...
    plot(audio, collected, args.wav, args.vad, args.whisper_model, out_path)
