...
class Wrp(object):
    VERSION = "Wrp/1.0.06 Python/%d.%d.%d (%s)" % (
        sys.version_info[0],
        sys.version_info[1],
        sys.version_info[2],
        sys.platform,
    ...
    @staticmethod
    def construct(implementation=0):
        if implementation == 0:
            implementation = 1  # default implementation
        if implementation == 1:
            from .Wrp_ import Wrp_

            return Wrp_()
        else:
            raise ValueError("Unknown implementation: " + u(str(implementation)))
    ...
    def __init__(self):
        ...
        self.maxDecodingTime_ = None
        self.maxResponseTime_ = None
        self.maxDecodingRate_ = None
        self.targetResponseTime_ = None
        self.targetDecodingRate_ = None
        self.noInputTimeout_ = None
        self.recognitionTimeout_ = None
    ...
    def setMaxDecodingTime(self, maxDecodingTime):
        self.maxDecodingTime_ = maxDecodingTime

    def setMaxResponseTime(self, maxResponseTime):
        self.maxResponseTime_ = maxResponseTime

    def setMaxDecodingRate(self, maxDecodingRate):
        self.maxDecodingRate_ = maxDecodingRate

    def setTargetResponseTime(self, targetResponseTime):
        self.targetResponseTime_ = targetResponseTime

    def setTargetDecodingRate(self, targetDecodingRate):
        self.targetDecodingRate_ = targetDecodingRate

    def setNoInputTimeout(self, noInputTimeout):
        self.noInputTimeout_ = noInputTimeout

    def setRecognitionTimeout(self, recognitionTimeout):
        self.recognitionTimeout_ = recognitionTimeout
    ...
    def setVoiceDetection(self, voiceDetection):
        if voiceDetection != None:
            ...
            if (
                len(voiceDetection) > 3
                and voiceDetection[0] == "G"
    ...
    def connect(self):
        ...
        try:
            ...
            self.connect_(
                self.serverURL_,
                self.proxyServerName_,
                self.connectTimeout_,
                self.receiveTimeout_,
            )
        except Exception as e:
            self.lastMessage_ = (
                "ERROR: can't connect to WebSocket server ("
                + u(str(e))
                + "): "
                + self.serverURL_
            )
    ...
    def feedDataResume(self):
        with self.condition_:
            if not self.isConnected_():
                self.lastMessage_ = (
                    "WARNING: already disconnected from WebSocket server"
                )
                if self.listener_ != None:
                    self.listener_.TRACE(self.lastMessage_)
                return False
            if self.state_ != 0:
                self.lastMessage_ = (
                    "WARNING: already started feeding data to WebSocket server"
                )
                if self.listener_ != None:
                    self.listener_.TRACE(self.lastMessage_)
                return False
            ...
            if self.grammarFileNames_ != None and len(self.grammarFileNames_) > 0:
                ...
                if self.grammarFileNames_.find(
                    "\001"
                ) != -1 and not self.grammarFileNames_.endswith("\001"):
                    command += "\001"
            else:
                command += " \001"
            if self.mode_ != None:
            ...
            if self.profileWords_ != None:
                ...
                if (
                    self.profileWords_.find(" ") != -1
                    or self.profileWords_.find('"') != -1
                ):
                    command += '"'
            ...
            if self.segmenterProperties_ != None:
                ...
                if (
                    self.segmenterProperties_.find(" ") != -1
                    or self.segmenterProperties_.find('"') != -1
                ):
                    command += '"'
            ...
            if self.resultUpdatedInterval_ != None:
                ...
                if self.resultUpdatedInterval_.find(" ") != -1:
                    ...
                    command += self.resultUpdatedInterval_
            if self.maxDecodingTime_ != None:
                command += " maxDecodingTime="
                if self.maxDecodingTime_.find(" ") != -1:
                    command += '"'
                    command += self.maxDecodingTime_
                    command += '"'
                else:
                    command += self.maxDecodingTime_
            if self.maxResponseTime_ != None:
                command += " maxResponseTime="
                if self.maxResponseTime_.find(" ") != -1:
                    command += '"'
                    command += self.maxResponseTime_
                    command += '"'
                else:
                    command += self.maxResponseTime_
            if self.maxDecodingRate_ != None:
                command += " maxDecodingRate="
                if self.maxDecodingRate_.find(" ") != -1:
                    command += '"'
                    command += self.maxDecodingRate_
                    command += '"'
                else:
                    command += self.maxDecodingRate_
            if self.targetResponseTime_ != None:
                command += " targetResponseTime="
                if self.targetResponseTime_.find(" ") != -1:
                    command += '"'
                    command += self.targetResponseTime_
                    command += '"'
                else:
                    command += self.targetResponseTime_
            if self.targetDecodingRate_ != None:
                command += " targetDecodingRate="
                if self.targetDecodingRate_.find(" ") != -1:
                    command += '"'
                    command += self.targetDecodingRate_
                    command += '"'
                else:
                    command += self.targetDecodingRate_
            if self.noInputTimeout_ != None:
                command += " noInputTimeout="
                if self.noInputTimeout_.find(" ") != -1:
                    command += '"'
                    command += self.noInputTimeout_
                    command += '"'
                else:
                    command += self.noInputTimeout_
            if self.recognitionTimeout_ != None:
                command += " recognitionTimeout="
                if self.recognitionTimeout_.find(" ") != -1:
                    command += '"'
                    command += self.recognitionTimeout_
                    command += '"'
                else:
                    command += self.recognitionTimeout_
            ...
            try:
                ...
                self.lastMessage_ = (
                    "WARNING: interrupted to start feeding data to WebSocket server"
                )
                if self.listener_ != None:
                    self.listener_.TRACE(self.lastMessage_)
                return False
    ...
    def feedData(self, data, dataOffset, dataBytes):
        with self.condition_:
            ...
            if self.state_ != 2 and self.state_ != 3:
                self.lastMessage_ = (
                    "WARNING: already stopped feeding data to WebSocket server"
                )
                if self.listener_ != None:
                    self.listener_.TRACE(self.lastMessage_)
                return False
            try:
                ...
                self.lastMessage_ = (
                    "ERROR: can't feed data to WebSocket server (" + u(str(e)) + ")"
                )
                if self.listener_ != None:
                    self.listener_.TRACE(self.lastMessage_)
                return False
    ...
    def feedDataPause(self):
        with self.condition_:
            ...
            try:
                ...
                self.lastMessage_ = (
                    "WARNING: interrupted to stop feeding data to WebSocket server"
                )
                if self.listener_ != None:
                    self.listener_.TRACE(self.lastMessage_)
                return False
    ...
    def onError_(self, cause):
        with self.condition_:
            if self.state_ == -1 or self.state_ == 0 or self.state_ == 5:
                return
    ...
    def onMessage_(self, message):
        ...
        if command == "s":
            ...
            if len(message) > 1:
                with self.condition_:
                    self.lastMessage_ = (
                        "ERROR: can't feed data to WebSocket server ("
                        + message[2:]
                        + ")"
                    )
                    if self.listener_ != None:
