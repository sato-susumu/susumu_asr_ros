...
class Wrp_(Wrp):
    ...
    def connect_(self, serverURL, proxyServerName, connectTimeout, receiveTimeout):
        ...
        if proxyServerName != None:
            if connectTimeout > 0:
                self.socket_ = socket.create_connection(
                    (proxyHostName, proxyPort), connectTimeout / 1000.0
                )
            else:
            ...
            if connectTimeout > 0:
                self.socket_ = socket.create_connection(
                    (hostName, port), connectTimeout / 1000.0
                )
            else:
        ...
        if proxyServerName != None:
            if scheme == "ws" or scheme == "wss":
                ...
                if (
                    ...
                    raise IOError(
                        "Invalid response code: " + proxyResponseHeader[9:12].decode()
                    )
            else:
                resource = serverURL
        if scheme == "wss":
            try:
                ...
                self.socket_ = context.wrap_socket(
                    self.socket_, server_hostname=hostName
                )
            except:
                self.socket_.close()
                self.socket_ = None
        ...
        if (
            ...
            raise IOError(
                "Handshake error (invalld response code: "
                + responseHeader[9:12].decode()
                + ")"
            )
    ...
    def disconnect_(self):
        if self.socket_ != None:
            try:
                self.outData_[0] = 0x88
                self.outData_[1] = 0x00
                self.socket_.send(self.outData_[:2])
    ...
    def sendRequest_(self, data, dataOffset, dataBytes, command):
        ...
        if command == 0:
            ...
            outDataBytes += 1
        self.outData_[outDataBytes : outDataBytes + dataBytes] = data[
            dataOffset : dataOffset + dataBytes
        ]
    ...
    def receiveResponse_(self):
        ...
        if not self.read_(2):
            if self.inDataBytes_ == 0:
                return False
            raise IOError("Unexpected end of stream")
        inDataType = self.inData_[0] & 0x0F
        inDataBytes = self.inData_[1] & 0x7F
        self.shift_(2)
        if inDataBytes == 126:
            if not self.read_(2):
                raise IOError("Unexpected end of stream")
            inDataBytes = ((self.inData_[0] & 0xFF) << 8) | (self.inData_[1] & 0xFF)
            self.shift_(2)
        ...
        if inDataType == 1:
            ...
            self.result_(
                0
            )  # set inDataBytes to 0 so that the data is not treated as a recognition result...
        else:
            raise IOError("Detected invalid frame data (data type " + inDataType + ")")
